...
export default function HabitTracker() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "analytics" | "check-in">("dashboard")
    ...
    setHabits(
      habits.map((habit) => {
        if (habit.id === id) {
          ...
          const todayIndex = habit.history.findIndex((day) => day.date === getTodayDate())
          ...
          if (newProgress >= habit.target) {
            newStreak += 1
          } else if (newProgress < habit.target && habit.progress >= habit.target) {
            newStreak = 0
          }
  ...
  // Components
  const Navbar = () => (
    <nav className={`fixed top-0 left-0 right-0 z-50 ${darkMode ? "bg-gray-900" : "bg-white"} shadow-md px-4 py-3`}>
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <motion.div initial={{ rotate: 0 }} animate={{ rotate: 360 }} transition={{ duration: 0.5 }}>
            <span className="text-2xl">📊</span>
          </motion.div>
          <h1 className={`text-xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>HabitTrack</h1>
        </div>
              ...
              >
                <div className="p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <span className={`block ${darkMode ? "text-white" : "text-gray-800"}`}>
                  ...
                  <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center space-x-3">
                      ...
                      />
                      <div>
                        <p className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>{userData.name}</p>
  ...
  const Dashboard = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>Your Habits</h2>
        <motion.button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {habits.map((habit) => (
          <motion.div
            key={habit.id}
            whileHover={{ y: -5 }}
            className={`p-4 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"} cursor-pointer`}
            onClick={() => handleViewHabitDetail(habit)}
          >
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center space-x-2">
                <span className="text-2xl">{habit.icon}</span>
                <h3 className={`font-semibold ${darkMode ? "text-white" : "text-gray-800"}`}>{habit.name}</h3>
              </div>
              <div
            ...
            <div className="mb-2">
              <div className="flex justify-between text-sm mb-1">
                <span className={`${darkMode ? "text-gray-300" : "text-gray-600"}`}>Today's Progress</span>
                  ...
                  return (
                    <div
                      ...
                      className={`w-4 h-4 rounded-sm ${
                        isCompleted ? "bg-green-500" : day ? "bg-red-400" : darkMode ? "bg-gray-700" : "bg-gray-200"
                      }`}
                    />
                  )
                })}
              </div>
              <span className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Last 7 days</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
          <h3 className={`text-xl font-bold mb-4 ${darkMode ? "text-white" : "text-gray-800"}`}>Sleep Tracking</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sleepData}>
                <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? "#374151" : "#e5e7eb"} />
                <XAxis dataKey="date" tickFormatter={formatDate} stroke={darkMode ? "#9ca3af" : "#6b7280"} />
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="text-center">
              <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Avg. Sleep</p>
            ...
            <div className="text-center">
              <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Deep Sleep</p>
              <p className={`text-xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>
                {sleepData[sleepData.length - 1].deepSleep}h
              </p>
            </div>
            <div className="text-center">
              <p className={`text-sm ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Sleep Quality</p>
              <p className={`text-xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>
        ...
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
          <h3 className={`text-xl font-bold mb-4 ${darkMode ? "text-white" : "text-gray-800"}`}>Screen Time</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {screenTimeData[screenTimeData.length - 1].categories.map((category, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: category.color }} />
                  <span className={`text-xs ${darkMode ? "text-gray-300" : "text-gray-600"}`}>{category.name}</span>
  ...
  const Analytics = () => (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h2 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>Analytics</h2>
        <div className="flex space-x-2">
          ...
          <button className={`px-3 py-1 text-sm rounded-md ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
            Month
          </button>
          <button className={`px-3 py-1 text-sm rounded-md ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
            Year
            ...
            <BarChart
              data={getWeekDays().map((day, index) => {
                const completedHabits = habits.filter((habit) => {
                  const historyDay = habit.history[habit.history.length - 7 + index]
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <PieChart>
                <Pie
                  data={habits.map((habit) => {
                    ...
                    const completionRate = Math.round((completedDays / habit.history.length) * 100)
        ...
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
          <h3 className={`text-xl font-bold mb-4 ${darkMode ? "text-white" : "text-gray-800"}`}>Water Intake</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={waterData}>
                ...
                <Line
                  ...
                  activeDot={{ r: 6 }}
                />
                <Line type="monotone" dataKey="target" stroke="#FFD166" strokeWidth={2} strokeDasharray="5 5" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
        <h3 className={`text-xl font-bold mb-4 ${darkMode ? "text-white" : "text-gray-800"}`}>Streak Performance</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <RadialBarChart
              cx="50%"
  ...
  const CheckIn = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>Daily Check-in</h2>
        <div
          ...
          {habits.map((habit) => (
            <div key={habit.id} className="space-y-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">{habit.icon}</span>
                  <span className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>{habit.name}</span>
                </div>
                <span
                  className={`text-sm font-medium ${
                    habit.progress >= habit.target ? "text-green-500" : darkMode ? "text-gray-400" : "text-gray-500"
                  }`}
                >
                  {habit.progress} / {habit.target} {habit.unit}
                </span>
              ...
              <div className="flex items-center space-x-4">
                <input
                  ...
                  onChange={(e) => handleUpdateProgress(habit.id, Number.parseInt(e.target.value))}
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleUpdateProgress(habit.id, Math.max(0, habit.progress - 1))}
                  ...
                  <button
                    onClick={() => handleUpdateProgress(habit.id, habit.progress + 1)}
                    className={`w-8 h-8 flex items-center justify-center rounded-full ${
                      darkMode
                        ? "bg-gray-700 text-gray-300 hover:bg-gray-600"
                        : "bg-gray-200 text-gray-600 hover:bg-gray-300"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className={`${darkMode ? "text-gray-300" : "text-gray-600"}`}>Hours Slept</span>
                <span className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  newSleepData[newSleepData.length - 1].hours = Number.parseFloat(e.target.value)
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className={`${darkMode ? "text-gray-300" : "text-gray-600"}`}>Sleep Quality</span>
                <span className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>
                  {sleepData[sleepData.length - 1].quality}%
                </span>
              </div>
              <input
                ...
                onChange={(e) => {
                  const newSleepData = [...sleepData]
                  newSleepData[newSleepData.length - 1].quality = Number.parseInt(e.target.value)
                  // This would update the state in a real app
                }}
                className="w-full h-2 bg-gray-300 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer"
              />
            ...
            <div className="relative w-32 h-32 mb-4">
              <svg className="w-full h-full" viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  strokeWidth="10"
                  strokeDasharray={`${
                    (waterData[waterData.length - 1].amount / waterData[waterData.length - 1].target) * 283
                  } 283`}
                  strokeLinecap="round"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>
                  {Math.round((waterData[waterData.length - 1].amount / waterData[waterData.length - 1].target) * 100)}%
                </span>
                <span className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>of daily goal</span>
              </div>
            </div>
        ...
        <div className={`p-6 rounded-lg shadow-md ${darkMode ? "bg-gray-800" : "bg-white"}`}>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className={`${darkMode ? "text-gray-300" : "text-gray-600"}`}>Today's Screen Time</span>
                <span className={`font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>
            ...
            <div>
              <p className={`text-sm mb-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Set Daily Limit</p>
              <div className="flex space-x-2">
                <button
                  className={`px-3 py-1 text-sm rounded-md ${
                    darkMode ? "bg-gray-700 text-white" : "bg-gray-200 text-gray-800"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className={`p-4 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}>
                  ...
                  <div className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-800"}`}>
                    {Math.round(
                      (selectedHabit.history.filter((day) => day.value >= selectedHabit.target).length /
              ...
              <div className="flex justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button
                  onClick={() => {
                    handleUpdateProgress(selectedHabit.id, selectedHabit.target)
                    setShowHabitDetail(false)
                    displayNotification(`Marked "${selectedHabit.name}" as completed!`)
                  }}
    ...
    <footer className={`mt-auto py-6 ${darkMode ? "bg-gray-900 text-gray-400" : "bg-gray-100 text-gray-600"}`}>
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="mt-4 md:mt-0 text-sm">&copy; {new Date().getFullYear()} HabitTrack. All rights reserved.</div>
        </div>
      </div>
    </footer>
  )
  ...
  return (
    <div className={`min-h-screen flex flex-col ${darkMode ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
      <Head>
        ...
        <meta name="description" content="Track your daily habits and personal stats" />
      </Head>
      <Navbar />
